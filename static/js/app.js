...
function animateCounters() {
  const counters = document.querySelectorAll('.stat-num[data-count], .stat-val[data-count]');
...
function initEchoesArchive() {
  ...
  const cards = document.querySelectorAll('.echo-interactive-card');
  ...
  const filterMap = {
    all: () => true,
    course: (card) => !['lab', 'professor', 'hostel', 'campus', 'placement'].some(t =>
  ...
  document.querySelectorAll('.expand-text-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const fullId = btn.dataset.targetFull;
      const teaserEl = document.getElementById(teaserId);
      ...
      if (isExpanded) {
        ...
        btn.querySelector('span').innerHTML = 'Read full memory &darr;';
      } else {
        fullEl.classList.remove('hidden');
        teaserEl.classList.add('hidden');
        btn.querySelector('span').innerHTML = 'Show less &uarr;';
      }
    });
  });
}
...
function initAudioPlayers() {
  document.querySelectorAll('.audio-play-trigger').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const echoId = btn.dataset.id;
      const waveformWrap = document.getElementById(`waveform-wrap-${echoId}`);
